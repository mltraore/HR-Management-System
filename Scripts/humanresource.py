...
from timex import Ui_Time as UTime
from searchId import Ui_SearchById as Ui_SI
from searchNM import Ui_searchByName as Ui_SN
...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        self.btnAddHome.clicked.connect(self.loadActivity)
        ...
        self.cmbPositionShowProfiles.addItem("")  # Position Combobox filled
        self.cmbPositionShowProfiles.addItem("HR Manager")
        self.cmbPositionShowProfiles.addItem("Finance Manager")
        self.cmbPositionShowProfiles.addItem("IT Personal")
        self.cmbPositionShowProfiles.addItem("Systems Analyst")
        self.cmbPositionShowProfiles.addItem("Project Manager")
        self.cmbPositionShowProfiles.addItem("Embedded Systems Engineer")
        self.cmbPositionShowProfiles.addItem("Software Engineer")
        self.cmbPositionShowProfiles.addItem("Accountant")
        self.cmbPositionShowProfiles.addItem("Sales Personnel")
        self.cmbPositionShowProfiles.addItem("All")


        self.cmbPositionShowProfiles.currentIndexChanged.connect(self.UpdateProfilesCombobox)
        ...
        self.lwShowProfiles.currentRowChanged.connect(self.updateProfile)
        ...
        self.lblProfileShowProfiles.setPixmap(
            QtGui.QPixmap("../../../../../../PycharmProjects/pythonProject/images/employee12.png"))
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../../../../../../PycharmProjects/pythonProject/images/icon1.png"),
        ...
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("../../../../../../PycharmProjects/pythonProject/images/browse.png"),
                        QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap("../../../../../../PycharmProjects/pythonProject/images/save.png"),
                        QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap("../../../../../../PycharmProjects/pythonProject/images/cancel.png"),
                        QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def Initialize(self, name, surname, tel, email, picture):
        self.lblNameHome.setText(name)
        self.lblSurnameHome.setText(surname)
        self.lblTelHome.setText(tel)
        self.lblEmailHome.setText(email)
        # self.lblProfileHome.setText(picture)
        # self.lblLogoHome(logo)

    def updateProfile(self):
        if int(str(self.lwShowProfiles.currentRow())) > -1:
            id_txt = self.lwShowProfiles.currentItem().text()
            id = int(id_txt.split(' ')[0])
            if "Manager" in self.cmbPositionShowProfiles.currentText():
                try:
                    con = sqlite3.connect('../DA/HRDatabase.db')
                    cur = con.cursor()
                    query = "SELECT identity, name, surname, phone, email, address, picture FROM managersTable WHERE identity='{}'".format(
                        id)
                    employee = cur.execute(query).fetchone()
                    self.lblIdentityShowProfiles.setText(str(employee[0]))
                    self.lblNameShowProfiles.setText(str(employee[1]))
                    self.lblSurnameShowProfiles.setText(str(employee[2]))
                    self.lblPhoneShowProfiles.setText(str(employee[3]))
                    self.lblEmailShowProfiles.setText(str(employee[4]))
                    self.lblAddressShowProfiles.setText(str(employee[5]))
                    self.lblProfileShowProfiles.setPixmap(QtGui.QPixmap("../Images/{}".format(str(employee[6]))))
    ...
    def UpdateProfilesCombobox(self):
        if self.cmbPositionShowProfiles.currentIndex() == 0:
            self.lwShowProfiles.clear()
        elif self.cmbPositionShowProfiles.currentIndex() == 1:
            self.lwShowProfiles.clear()
            try:
                con = sqlite3.connect('../DA/HRDatabase.db')
                cur = con.cursor()
                query = "SELECT identity, name, surname FROM managersTable WHERE position='{}'".format("HR Manager")
                employees = cur.execute(query).fetchall()
                for employee in employees:
                    self.lwShowProfiles.addItem(
                        str(employee[0]) + "  -  " + str(employee[1]) + "    " + str(employee[2]))

            except sqlite3.Error as e:
                print(e)
            finally:
                con.close()
        elif self.cmbPositionShowProfiles.currentIndex() == 2:
            self.lwShowProfiles.clear()
            try:
                con = sqlite3.connect('../DA/HRDatabase.db')
                cur = con.cursor()
                query = "SELECT identity, name, surname FROM managersTable WHERE position='{}'".format(
                    "Finance Manager")
                employees = cur.execute(query).fetchall()
                for employee in employees:
                    self.lwShowProfiles.addItem(
        ...
        elif self.cmbPositionShowProfiles.currentIndex() == 3:
            pass
        elif self.cmbPositionShowProfiles.currentIndex() == 4:
            pass
        elif self.cmbPositionShowProfiles.currentIndex() == 5:
            pass
        elif self.cmbPositionShowProfiles.currentIndex() == 6:
            pass
    ...
    def loadActivity(self):
        ...
        self.window.show()
        self.window.exec_()
        if self.ui.lnActivityTime.text() != "":
            self.lwTimeTableHome.addItem(
                self.ui.lnActivityTime.text() + " (" +
                self.ui.dateEditTime.text() + ", " +
                self.ui.timeEditStartTime.text() + "-" +
                self.ui.timeEditFinishTime.text() + ")"
            )
        else:
            # self.lwTimeTableHome.removeItemWidget()
            # for SelectedItem in self.lwTimeTableHome.ContentList.selectedItems():
            #     self.lwTimeTableHome.ContentList.takeItem(self.lwTimeTableHome.ContentList.row(SelectedItem))
            self.window.close()
    ...
    def retranslateUi(self, Dialog):
        ...
        self.btnDeleteHome.clicked.connect(self.deleteSelected)
        self.btnDeleteAllHome.clicked.connect(self.deleteAll)
        ...
        self.btnShowAllShowProfiles.setText(_translate("Dialog", "Show All"))  # Show All button
        ...
        self.btnSearchByIdShowProfiles.clicked.connect(self.searchID)
        self.btnSearchByNameShowProfiles.clicked.connect(self.searchName)
        ...
        # self.lblIdentityShowProfiles.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)
    ...
    def deleteSelected(self):
        if int(str(self.lwTimeTableHome.currentRow())) > -1:
            self.lwTimeTableHome.removeItemWidget(self.lwTimeTableHome.takeItem(self.lwTimeTableHome.currentRow()))
        else:
            QtWidgets.QMessageBox.information(Dialog, "Delete Error", "No item selected")

    def deleteAll(self):
        if self.lwTimeTableHome.count() > 0:
            self.lwTimeTableHome.clear()
        else:
            QtWidgets.QMessageBox.information(Dialog, "Delete Error", "Empty List")

    def searchID(self):
        self.window_s = QtWidgets.QDialog()
        self.ui_s = Ui_SI()
        self.ui_s.setupUi(self.window_s)
        self.window_s.show()
        self.window_s.exec_()
        self.e_id = int(self.ui_s.lineEdit.text()) if self.ui_s.lineEdit.text() != "" else "0"
        ...
        if self.e_id != 0:
            ...
            try:
                con = sqlite3.connect('../DA/HRDatabase.db')
                cur = con.cursor()
                query = "SELECT identity, name, surname FROM managersTable WHERE identity={}".format(self.e_id)
                employee = cur.execute(query).fetchone()
                if employee is not None:
                    ...
                    query = "SELECT identity, name, surname FROM employeeTable WHERE identity={}".format(self.e_id)
                    employee = cur.execute(query).fetchone()
                    if employee is not None:
                        self.lwShowProfiles.addItem(
                            str(employee[0]) + "  -  " + str(employee[1]) + "    " + str(employee[2])
                        ...
                        QtWidgets.QMessageBox.information(Dialog, "Not Found", "No such Employee")
            except sqlite3.Error as e:
                print(e)
    ...
    def searchName(self):
        self.window_n = QtWidgets.QDialog()
        self.ui_n = Ui_SN()
        self.ui_n.setupUi(self.window_n)
        self.window_n.show()
        self.window_n.exec_()
        self.e_name = self.ui_n.lineEdit.text().split(' ')[0]
        ...
        if self.e_name != "":
            ...
            try:
                con = sqlite3.connect('../DA/HRDatabase.db')
                cur = con.cursor()
                query = "SELECT identity, name, surname FROM managersTable WHERE name='{}'".format(self.e_name)
                employee = cur.execute(query).fetchone()
                if employee is not None:
                    self.lwShowProfiles.addItem(
                        str(employee[0]) + "  -  " + str(employee[1]) + "    " + str(employee[2])
                    ...
                    query = "SELECT identity, name, surname FROM employeeTable WHERE name='{}'".format(self.e_name)
                    employee = cur.execute(query).fetchone()
                    if employee is not None:
                        self.lwShowProfiles.addItem(
                            str(employee[0]) + "  -  " + str(employee[1]) + "    " + str(employee[2])
            ...
            QtWidgets.QMessageBox.information(Dialog, "Not Found", "Empty Name")
        else:
            self.window_n.close()
